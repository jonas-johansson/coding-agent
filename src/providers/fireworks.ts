...
export class FireworksProvider implements Provider {
  ...
  constructor() {
    ...
    this.apiKey = key;
    this.baseUrl = process.env.FIREWORKS_BASE_URL ?? DEFAULT_BASE_URL;
  }
  ...
  async stream(params: {
    ...
    const body = {
      ...
      stream: true,
      // Fireworks includes usage in the final streamed chunk automatically,
      // but we also request it explicitly for forward compatibility.
    ...
    if (!response.ok) {
      ...
      throw new Error(`Fireworks AI request failed (${response.status}): ${text.slice(0, 500)}`);
    }

    if (!response.body) {
      throw new Error("Fireworks AI response has no body");
    }

    return new FireworksStream(response.body.getReader());
  }
}
...
class FireworksStream implements ProviderStream {
